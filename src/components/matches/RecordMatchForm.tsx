...
const recordMatchSchema = z.object({
  ...
  teamAScore: z.number().min(0).max(MAX_SCORE),
  teamBScore: z.number().min(0).max(MAX_SCORE),
  playerGoals: z.array(playerGoalSchema),
}).refine(data => data.teamAScore === MAX_SCORE || data.teamBScore === MAX_SCORE, {
  message: `One team's score must be ${MAX_SCORE}.`,
  path: ["teamAScore"], // Or teamBScore, path doesn't matter much here
}).refine(data => data.teamAScore !== data.teamBScore, {
  message: "Scores cannot be a draw if one team reached 10.",
  path: ["teamAScore"],
});
...
export function RecordMatchForm() {
  ...
  const [availableMatchups, setAvailableMatchups] = useState<GeneratedMatchup[]>([]);
  const [selectedMatchup, setSelectedMatchup] = useState<GeneratedMatchup | null>(null);
  const [allPlayersInMatch, setAllPlayersInMatch] = useState<Player[]>([]);

  const { control, handleSubmit, watch, reset, setValue, formState: { errors } } = useForm<RecordMatchFormData>({
  ...
  useEffect(() => {
    if (selectedMatchupId) {
      ...
      if (matchup) {
        const teamAPlayers = matchup.teamA.playerIds.map(id => players.find(p => p.id === id)).filter(Boolean) as Player[];
        const teamBPlayers = matchup.teamB.playerIds.map(id => players.find(p => p.id === id)).filter(Boolean) as Player[];
        const currentMatchPlayers = [...teamAPlayers, ...teamBPlayers];
        setAllPlayersInMatch(currentMatchPlayers);
        
        // Initialize playerGoals based on selected matchup
        const initialGoals = currentMatchPlayers.map(player => ({
          playerId: player.id,
          goals: 0,
        }));
        replace(initialGoals); // Use replace to set the entire array
      } else {
  ...
  const onSubmit = (data: RecordMatchFormData) => {
    if (!selectedMatchup) {
      toast({ title: "Error", description: "No matchup selected.", variant: "destructive" });
      return;
    }
  ...
  if (players.length < 5) {
    return (
      <div className="flex items-center gap-2 p-4 border border-yellow-500/50 bg-yellow-500/10 rounded-md text-yellow-700 dark:text-yellow-300">
        ...
        <p>Not enough players to form standard 2v3 teams. Please ensure there are 5 players available.</p>
      </div>
    );
  }
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div>
          ...
          />
          {errors.selectedMatchupId && <p className="text-sm text-destructive mt-1">{errors.selectedMatchupId.message}</p>}
        </div>
      </div>
        ...
        <Card>
          <CardContent className="p-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-end">
              <div>
                <Label htmlFor="teamAScore">{selectedMatchup.teamA.name} Score</Label>
                <Controller
                  name="teamAScore"
                  control={control}
                  render={({ field }) => <Input type="number" {...field} onChange={e => field.onChange(parseInt(e.target.value) || 0)} />}
                />
                {errors.teamAScore && <p className="text-sm text-destructive mt-1">{errors.teamAScore.message}</p>}
              </div>
              <div>
                <Label htmlFor="teamBScore">{selectedMatchup.teamB.name} Score</Label>
                <Controller
                  name="teamBScore"
                  control={control}
                  render={({ field }) => <Input type="number" {...field} onChange={e => field.onChange(parseInt(e.target.value) || 0)} />}
                />
                 {errors.teamBScore && <p className="text-sm text-destructive mt-1">{errors.teamBScore.message}</p>}
              </div>
            </div>
             {errors.root?.message && <p className="text-sm text-destructive mt-2">{errors.root.message}</p>}
          </CardContent>
        </Card>
      )}
        ...
        <div>
          ...
          <ScrollArea className="h-[200px] rounded-md border p-4">
            ...
            {fields.map((item, index) => {
              const player = allPlayersInMatch.find(p => p.id === item.playerId);
              return player ? (
                <div key={item.id} className="flex items-center justify-between">
                  <Label htmlFor={`playerGoals.${index}.goals`}>{player.name}</Label>
                  <Controller
                    name={`playerGoals.${index}.goals`}
                    control={control}
                    render={({ field }) => (
                      <Input type="number" className="w-20" {...field} onChange={e => field.onChange(parseInt(e.target.value) || 0)} />
                    )}
                  />
                </div>
              ) : null;
          ...
          {errors.playerGoals && <p className="text-sm text-destructive mt-1">{errors.playerGoals.message || errors.playerGoals.root?.message}</p>}
        </div>
      )}

      <Button type="submit" size="lg" className="w-full md:w-auto">Record Match</Button>
    </form>
  );
}
