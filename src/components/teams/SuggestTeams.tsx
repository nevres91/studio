...
import { suggestTeams, type SuggestTeamsInput, type SuggestTeamsOutput } from '@/ai/flows/suggest-teams'; // Assuming AI flow is correctly exported
  ...
  const handleSuggestTeams = async () => {
    if (players.length < 5) {
      toast({
        ...
        description: "AI suggestions require at least 5 players with game data.",
        variant: "destructive",
      });
      return;
    }
    ...
    if (playersWithSufficientData.length < 5) {
         toast({
            ...
            description: "Not all players have played matches. AI needs more data for accurate suggestions.",
            variant: "destructive"
        });
    ...
    <div className="space-y-6">
      <div className="flex flex-col items-center text-center">
        ...
        <p className="text-muted-foreground mb-4">
          Let AI analyze player stats and propose fair teams for your next game. This considers total goals, win/loss ratios, and games played.
        </p>
        <Button onClick={handleSuggestTeams} disabled={isLoading || players.length < 5} size="lg">
          {isLoading ? (
        ...
        {players.length < 5 && (
          <p className="text-sm text-destructive mt-2 flex items-center">
            <AlertTriangle className="h-4 w-4 mr-1"/> Requires 5 players to be configured.
          </p>
        )}
      </div>
        ...
        <Card className="bg-background shadow-xl animate-in fade-in duration-500">
          <CardHeader>
            <CardTitle className="text-xl flex items-center">
              <Users className="mr-2 h-5 w-5 text-primary" /> AI Suggested Teams
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <Card className="border-primary/50">
                <CardHeader>
                  <CardTitle className="text-lg text-primary">Team A</CardTitle>
                </CardHeader>
                <CardContent>
                  <ul className="list-disc list-inside">
                    {suggestion.teamA.map(playerName => <li key={playerName}>{playerName}</li>)}
                  </ul>
                </CardContent>
              </Card>
              <Card className="border-accent/50">
                <CardHeader>
                  <CardTitle className="text-lg text-accent">Team B</CardTitle>
                </CardHeader>
                <CardContent>
                  <ul className="list-disc list-inside">
                    {suggestion.teamB.map(playerName => <li key={playerName}>{playerName}</li>)}
                  </ul>
                </CardContent>
              </Card>
            </div>
            <div>
              ...
              <p className="text-sm text-muted-foreground p-3 bg-secondary/50 rounded-md">{suggestion.reasoning}</p>
            </div>
          </CardContent>
        </Card>
      )}
